...
function exportCSV(submissions: ConsultSubmission[]) {
  ...
  const rows = submissions.map((s) => [
    ...
    s.selectedProducts?.length ? s.selectedProducts.map((p) => `${p.name}(${p.model})`).join(" | ") : (s.purpose ?? ""),
    s.area ?? "",
    s.apartment ?? "",
    (s.channels ?? []).join(" | "),
    s.model ?? "",
...
export default function ConsultAdmin() {
  ...
  useEffect(() => {
    adminStore.consult.get().then((data) => {
      setSubmissions(data);
      setLoading(false);
    });
  }, []);
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-[#888]">
            총 {submissions.length}건 · 신규 {submissions.filter((s) => s.status === "new").length}건
          </p>
        </div>
        <button
      ...
      {submissions.length === 0 ? (
        ...
        <div className="space-y-3">
          {submissions.map((sub) => (
            <div key={sub.id} onClick={() => openDetail(sub)} className="flex cursor-pointer items-center justify-between rounded-2xl bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[13px] text-[#888]">
                  {sub.selectedProducts?.length
                    ? `${sub.selectedProducts.length}개 제품 · ${sub.selectedProducts
                        .slice(0, 2)
                        .map((p) => `${p.name}${p.selectedColor ? `/${p.selectedColor.name}` : ""}`)
                        .join(" · ")}${sub.selectedProducts.length > 2 ? " 외" : ""}`
                    : `${sub.purpose ?? ""} · ${sub.area ?? ""}`}
                </p>
                <div className="flex items-center gap-3">
                  <p className="text-[11px] text-[#bbb]">{new Date(sub.submitted_at).toLocaleString("ko-KR")}</p>
                  {sub.memo && <p className="truncate max-w-50 text-[11px] text-[#aaa]">📝 {sub.memo}</p>}
                </div>
              </div>
              <span className={`ml-3 shrink-0 rounded-full border px-3 py-1 text-[12px] font-semibold ${STATUS[sub.status].cls}`}>{STATUS[sub.status].label}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-white mx-4">
            ...
            <div className="flex items-center justify-between border-b border-[#f1f1f1] px-6 py-4">
              ...
              <button onClick={() => setSelected(null)} className="flex h-8 w-8 items-center justify-center rounded-full text-[18px] text-[#888] hover:bg-[#f5f5f5]">
                ✕
              </button>
            </div>
            ...
            <div className="overflow-y-auto px-6 py-4 space-y-4">
              <div className="space-y-3">
                {selected.selectedProducts?.length ? (
                  <>
                    <div className="rounded-xl border border-[#f0f0f0] bg-[#fafafa] p-3 space-y-2">
                      {selected.selectedProducts.map((p) => (
                        <div key={p.id} className="rounded-lg border border-[#ececec] bg-white p-3">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-[13px] font-semibold text-[#1a1a1a]">{p.name}</p>
                              <p className="mt-0.5 text-[12px] text-[#999]">{p.model}</p>
                            </div>
                            {p.image ? (
                              <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-[#f7f7f7]">
              ...
              <div className="rounded-xl border border-[#f0f0f0] bg-[#fafafa] p-4">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <button onClick={saveMemo} disabled={memoSaving} className="h-8 rounded-full bg-[#1a1a1a] px-4 text-[12px] font-bold text-white hover:opacity-80 disabled:opacity-50">
...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2">
      <span className="w-20 shrink-0 text-[11px] font-semibold text-[#888]">{label}</span>
      <span className="text-[12px] text-[#333]">{value}</span>
    </div>
  );
}
