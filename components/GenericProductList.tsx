...
function TagBadge({ tag }: { tag: ManagedProduct["tags"][number] }) {
  const styles: Record<string, string> = {
    md: "bg-[#c90f45] text-white",
    new: "bg-[#00b14f] text-white",
...
function ProductCard({ product, section }: { product: ManagedProduct; section: string }) {
  const [imgError, setImgError] = useState(false);

  return (
    <a href={`/products/${section}/${product.id}`} className="group flex flex-col">
...
function BestCarousel({ bestProducts, section, label }: { bestProducts: ManagedProduct[]; section: string; label: string }) {
  if (bestProducts.length === 0) return null;

  return (
    <section className="bg-[#f5f5f5] py-8 sm:py-10">
      <div className="mx-auto max-w-300 px-5">
        <h2 className="mb-6 text-[18px] font-black tracking-[-0.04em] text-[#1a1a1a] sm:text-[20px] lg:text-[22px]">{label} 베스트</h2>
...
export default function GenericProductList({ section }: { section: string }) {
  ...
  useEffect(() => {
    productStore.products.getBySection(section).then(setProducts);
    productStore.categories.getBySection(section).then((cats) => setCategories(cats.map((c) => c.name)));
  ...
  return (
    <>
      <BestCarousel bestProducts={products.filter((p) => p.isBest)} section={section} label={label} />

      <div className="mx-auto max-w-300 px-5 py-8 sm:py-10">
        <h2 className="mb-6 text-[18px] font-black tracking-[-0.04em] text-[#1a1a1a] sm:text-[20px] lg:text-[22px]">{label}</h2>
