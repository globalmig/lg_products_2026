...
export const adminStore = {
  slides: {
    get: async (): Promise<(Slide & { image_key: string })[]> => {
      try {
        const rows = await apiFetch<{ id: number; image_key: string; subtitle: string; title: string; link: string }[]>("/api/slides");
        return rows.map((r) => ({ id: r.id, image: imageUrl(r.image_key), image_key: r.image_key, subtitle: r.subtitle, title: r.title, link: r.link }));
      } catch {
        return DEFAULT_SLIDES.map((s) => ({ ...s, image_key: s.image }));
      }
    },
    add: (slide: { image_key: string; subtitle: string; title: string; link?: string; sort_order?: number }) =>
      apiFetch("/api/slides", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(slide) }),
    update: (id: number, slide: { image_key: string; subtitle: string; title: string; link?: string; sort_order?: number }) =>
